...
export function setupCrazyGame(shadowRoot: ShadowRoot | null) {
    ...
    function handleCrazyPlayerMovement(gameState: CrazyGameState, keys: Record<string, boolean>) {
        ...
        if (keys['w']) gameState.players.left.y = Math.max(0, gameState.players.left.y - moveSpeed);
        if (keys['s']) gameState.players.left.y = Math.min(maxPos, gameState.players.left.y + moveSpeed);
        
        // Jugador derecho (flecha arriba/abajo)
        if (keys['arrowup']) gameState.players.right.y = Math.max(0, gameState.players.right.y - moveSpeed);
        if (keys['arrowdown']) gameState.players.right.y = Math.min(maxPos, gameState.players.right.y + moveSpeed);
        
        // Jugador superior (Y/U)
        const maxPosHorizontal = boardSize - gameState.players.top.width;
        if (keys['y']) gameState.players.top.x = Math.max(0, gameState.players.top.x - moveSpeed);
        if (keys['u']) gameState.players.top.x = Math.min(maxPosHorizontal, gameState.players.top.x + moveSpeed);
        
        // Jugador inferior (B/N)
        if (keys['b']) gameState.players.bottom.x = Math.max(0, gameState.players.bottom.x - moveSpeed);
        if (keys['n']) gameState.players.bottom.x = Math.min(maxPosHorizontal, gameState.players.bottom.x + moveSpeed);
    }

    function updateCrazyBallPosition(gameState: CrazyGameState) {
        const ball = gameState.ball;
        ball.x += ball.speedX;
        ball.y += ball.speedY;
    }

    function checkCrazyPaddleCollisions(gameState: CrazyGameState) {
        const ball = gameState.ball;
        const boardSize = 800;
        const ballRadius = 8;
    ...
    function checkCrazyLives(gameState: CrazyGameState) {
        const ball = gameState.ball;
        const boardSize = 800;

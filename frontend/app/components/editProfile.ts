			...
			saveButton.addEventListener("click", async () => {
				...
				if (twofaCheckbox.checked && !this.response.twofa) {
				  try {
					...
					const res = await fetch(`https://localhost:8443/api/2fa/setup`, {
					  method: "POST",
					  credentials: "include",
					});
					...
					try {
  					const res = await fetch(`https://localhost:8443/api/debug-token`, {
    				credentials: "include",
					});
				...
				// Auth check
				try {
  				const authRes = await fetch(`https://localhost:8443/api/test-auth`, {
    			credentials: "include",
  			});
				...
				const verifyRes = await fetch(`https://localhost:8443/api/2fa/verify`, {
					  method: "POST",
				...
				if (!twofaCheckbox.checked && this.response.twofa) {
				  ...
				  try {
  					const res = await fetch(`https://localhost:8443/api/2fa/disable`, {
    				method: "POST",
    				credentials: "include",
    				headers: {
				...
				try {
					const response = await fetch(`https://localhost:8443/api/edit-profile`, {
						method: "POST",
						body: formData,
						credentials: "include",
					});

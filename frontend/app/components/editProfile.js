...
class EditProfileComponent extends HTMLElement {
    ...
    render() {
        ...
        const avatarUrl = `https://localhost:8443/api/static/${avatar}`;
            ...
            saveButton.addEventListener("click", () => __awaiter(this, void 0, void 0, function* () {
                ...
                if (twofaCheckbox.checked && !this.response.twofa) {
                    try {
                        ...
                        const res = yield fetch(`https://localhost:8443/api/2fa/setup`, {
                            method: "POST",
                            credentials: "include",
                        });
                        if (!res.ok)
                        ...
                        // QR Code window
                        const userCode = yield this.showQrModal(data.qrCode);
                        if (!userCode) {
                            alert("2FA activation cancelled.");
                            return;
                        }
                        try {
                            const res = yield fetch(`https://localhost:8443/api/debug-token`, {
                                credentials: "include",
                            });
                            const data = yield res.json();
                        ...
                        // Auth check
                        try {
                            const authRes = yield fetch(`https://localhost:8443/api/test-auth`, {
                                credentials: "include",
                            });
                            const authData = yield authRes.json();
                        ...
                        // 2FA verification
                        const verifyRes = yield fetch(`https://localhost:8443/api/2fa/verify`, {
                            method: "POST",
                ...
                if (!twofaCheckbox.checked && this.response.twofa) {
                    const password = yield this.askPassword();
                    if (!password)
                        return;
                    try {
                        const res = yield fetch(`https://localhost:8443/api/2fa/disable`, {
                            method: "POST",
                            credentials: "include",
                            headers: {
                ...
                try {
                    const response = yield fetch(`https://localhost:8443/api/edit-profile`, {
                        method: "POST",
                        body: formData,
                        credentials: "include",
                    });
                    if (response.ok) {
                        ...
                        yield this.getProfile(); // Refresh profile
